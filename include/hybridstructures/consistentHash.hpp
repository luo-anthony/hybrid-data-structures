...
template <typename T>
ConsistentHashRing<T>::ConsistentHashRing(int nReps): replicationFactor{nReps} { 
    replicationFactor = std::max(minReplicas, replicationFactor);
}

template <typename T>
std::string ConsistentHashRing<T>::nodeToString(const T& node){
...
template <typename T>
int ConsistentHashRing<T>::size() {
    return internalNodes.size();
}

template <typename T>
bool ConsistentHashRing<T>::containsNode(const T& node) {
    size_t nodeHash = hashFunc(nodeToString(node));
    return (internalNodes.count(nodeHash) == 1);
}

template <typename T>
std::vector<T> ConsistentHashRing<T>::getAllNodes(){
...
template <typename T>
void ConsistentHashRing<T>::addNode(const T& node) {
...
template <typename T>
bool ConsistentHashRing<T>::removeNode(const T& node) {
    if(!containsNode(node)){
        return false;
    }
...
template <typename T>
const T& ConsistentHashRing<T>::locateKey(const std::string& key) {
    if(this->size() == 0){
...
// NOTE: this function is currently bugged, might return duplicate nodes since this only looks through virtual nodes 
template <typename T>
